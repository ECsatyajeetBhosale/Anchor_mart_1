...
# Contact channels — add and verify email / WhatsApp

Added 2026-08-10. **Sailors and delivery partners share these four endpoints**
(`user/contact_views.py`); admin-tier accounts get **403**.

## The gap they close

Sign-up proves **one** channel and only one — the email path sets `email_verified`, the
WhatsApp path sets `whatsapp_verified` and explicitly leaves the other False ("the two channels
are never inferred from each other"). There was then no way to add the second:
`UserProfileUpdateSerializer` cannot write `email` (read-only) or `whatsapp_number` (not in its
fields).

Worse, the obvious workaround was actively harmful. `POST /api/v1/user/whatsapp/verify-otp/` is
a **sign-in** endpoint: it resolves an account *by number* and creates one when none exists. A
signed-in sailor calling it received **200**, a brand-new duplicate account for their number,
and no change to their own row — so the app reloaded the profile, saw `whatsapp_verified:
false`, and reported "verification failed".

> **These are on separate paths from the sign-in pair for exactly that reason.** They read the
> account from the token, create nothing, and issue no session. The caller stays signed in as
> themselves and no token is returned.

## Rules that apply to all four

- **An identifier is only ever written after an OTP proves it.** That is why `email` and
  `whatsapp_number` are absent from the profile-update serializer — this is their only writer.
- **Uniqueness is checked twice**, at send *and* at verify. The gap between them is a real
  window, and `uniq_user_email_ci` / `uniq_user_whatsapp_pair` are the backstop: a lost race
  surfaces as the same **409**, never a 500.
- **The caller is never in conflict with themselves** — re-confirming your own address is fine.
- **Admins are refused (403).** An admin's email is a password-login credential; it changes
  through the admin-user-management module (Flow 31), which has its own guards.
- Existing OTP cooldowns and the per-code attempt cap apply unchanged.

## API 18 · `POST /api/v1/user/contact/email/send-otp/`

**Body** — `{"email": "new@example.com"}`. Sends a code to the address being claimed.

The uniqueness check runs **before** the OTP is spent: sending first would also tell the holder
of that address that somebody is trying to claim it.

**200** `{"message": "OTP sent to this email address."}`
**Errors** — 400 · **409** (registered to another account) · **429** (resend cooldown) · 403 (admin)

## API 19 · `POST /api/v1/user/contact/email/verify-otp/`

**Body** — `{"email": "new@example.com", "otp": "1234"}`. The address is re-sent with the code
because the OTP is keyed on the **address**, not on the session.

On success writes `email` (lower-cased) and `email_verified = True`.

**200**
```json
{ "message": "Email verified successfully.",
  "user": { "email": "new@example.com", "email_verified": true,
            "country_code": "+91", "whatsapp_number": "9876543210",
            "whatsapp_verified": true } }
```
**Errors** — 400 (wrong / expired / no active code) · **409** · **429** (attempt cap) · 403

## API 20 · `POST /api/v1/user/contact/whatsapp/send-otp/`

**Body** — `{"country_code": "+91", "whatsapp_number": "9876543210"}`. `country_code` is
required unless the number is already full E.164 (`+…`).

**200** `{"message": "OTP sent to this WhatsApp number."}`
**Errors** — 400 · **409** · **429** · 403

## API 21 · `POST /api/v1/user/contact/whatsapp/verify-otp/`

**Body** — `{"country_code": "+91", "whatsapp_number": "9876543210", "otp": "1234"}`.

On success writes `country_code`, `whatsapp_number` (both normalised as `User.save()` stores
them) and `whatsapp_verified = True`. Same response envelope as API 19.

**Errors** — 400 · **409** · **429** · 403

## Profile reporting

`GET /api/v1/user/get-profile/` now returns **`email_verified`** alongside
`whatsapp_verified` — it was missing, so a client could see that a *number* needed verifying
but never an *address*. Gate each channel's verify button on its own flag.

> **Known debt.** WhatsApp OTP *verification* lives inline in the frozen sign-in view with no
> reusable helper, so `contact_service.consume_whatsapp_otp` mirrors it step for step. Two
> copies of an attempt-cap will not stay in sync; unifying them needs sign-off to touch the
> frozen OTP path.
