...
| **Endpoints** | **9** — 8 under `/api/notifications/` (+ FCM token register under `/api/v1/user/`) — fully specified below |
...
## 2 · `GET /api/notifications/<uuid:notification_id>/` — One notification in full

The detail screen behind an inbox row or a push tap. Added 2026-08-10.

**Returns the feed row's fields, plus four the list has no room for:**

| Field | Notes |
|---|---|
| `status` | The **target's** current status (`payment_pending`, `delivered`…). Already computed alongside `action_required`; the row simply does not show it. |
| `image` | Admin-attached banner, or null. Absent from the list on purpose — 20 rows would each build a URL no feed renders. |
| `read_at` | Formatted timestamp, null until read. The list carries only the derived `is_read` boolean. |
| `updated_at` | Display format, per §1. |

`metadata` is deliberately **not** returned: it is an internal bag (`{order_id, payment_id}`)
and the client already gets a typed deep-link in `action`.

**It does NOT mark the notification read.** A GET with that side effect makes "unread"
un-observable, and API 3 already exists and is idempotent.

**Same visibility rule as the feed.** Both read `_inbox_qs(user)` — scoped to the caller,
`type__in=_FEED_TYPES` — so a row you can see in the list is fetchable by id, and a type
outside the inbox feed (a `PROMO`, which exists in the table but was never an inbox item)
**404s** rather than being reachable through a guessed URL.

`action_required` / `action` / `status` are derived from the target at read time, exactly as in
the feed: opening a stale "Pay Now" row after the order is paid reports it inert, with nothing
mutated to make that true.

**Errors** — 401 · **404** for another user's id, an unknown id, or a non-feed type (never 403 —
the response must not confirm the row exists) · a malformed UUID 404s at the router.

---

## 3 · `POST /api/notifications/<uuid:notification_id>/read/` — Mark one read
...
## 6 · `PUT | PATCH /api/notifications/preferences/update/` — Update preferences
...
## 7 · `GET /api/notifications/unsubscribe/?token=&email=` — One-click email unsubscribe
...
## 8 · `GET /api/notifications/resubscribe/?token=&email=` — One-click resubscribe
...
## 9 · `POST /api/v1/user/add-fcm-token/` — Register a device for push
