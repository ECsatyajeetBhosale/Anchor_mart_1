...
| **Documentation Version** | 1.1 — 2026-08-07 (FE1, shipped 2026-08-06: the quote guard became the `Product.is_internal` column; `not_a_quote_*_q()` → `public_products_q()` / `public_variants_q()`). 1.0 — 2026-07-27 |
...
**Internal-product safety gate.** *(Rewritten 2026-08-06 — FE1.)* Some products must never reach a
sailor: a private special-request **quote product** (a bespoke item created for one sailor, priced for
them), and a **partner-substitute holding product** (the internal reuse pool that promoted partner
substitutes are parented under). Both are marked **`Product.is_internal = True` by whatever created
them**; everything else is public. Every waitlist surface excludes them — `public_products_q()` over a
Product queryset, `public_variants_q()` over a Variant queryset — so a crafted `variant_id` can't
waitlist, and later leak, someone else's private quote.

> **These two functions were renamed on 2026-08-06** — they were `not_a_quote_product_q()` /
> `not_a_quote_variant_q()`, named for what they blocked when quotes were the only hidden kind.
> They are now named for what they **admit**, since `is_internal` covers two kinds of hidden product.
>
> The rename came with a **change of mechanism, not just a name**. The old gate was a subquery —
> "does any `SuggestedProductByAdmin` point at this product?" — but that row is written by two
> operations with opposite intent: `suggest_new_product` (genuinely bespoke, the intended target)
> and `suggest_replacement`, which points at an **existing catalog variant**. So the first time an
> admin offered catalog SKU *X* as a replacement on any order, *X*'s whole parent product and every
> sibling variant vanished from browse for every sailor — permanently, since nothing deletes the
> suggestion row and `variant` is `on_delete=PROTECT`. The discriminator had to become **how the
> product was created**, which is what the `is_internal` column records.

`admin_sourceable=False` remains **overloaded** for its other meaning (out-of-sourcing vs. hidden);
see [[admin-sourceable-overloaded]].
...
nobody can buy): `is_deleted=False`, `is_active=True`, **not an internal product**
(`public_products_q()` — quote products and partner-substitute holding products), **`admin_sourceable=True`**,
**and** at least one live variant with `admin_sourceable=True`.
...
### `POST /api/superadmin/product-variants/set-admin-sourceable/<variant_id>/`
