...
**Error responses — flow-wide.** Every endpoint here answers from this set; per-API sections
note only what is specific to them.

| Status | When | Body |
|---|---|---|
| **400** | Field validation — blank `reason`, bad `tab`, an invalid `to_status`, a non-billable `order_item_id`, a `photo` outside the `proof_of_delivery/` prefix | DRF field map, e.g. `{"tab": ["Must be one of: all, delivered, …"]}` |
| **401** | Missing or expired token | `{"detail": "Authentication credentials were not provided."}` |
| **403** | Authenticated but not a delivery partner, or missing `server-secret-key` | `{"detail": "Only delivery partners can access this resource."}` |
| **404** | The order is not the caller's active assignment — **never 403**, so the response cannot confirm the order exists | `{"detail": "Not found."}` |
| **409** | A state-machine refusal (`transition_order`), an unpaid delta blocking handover, or an out-of-order transit stage | `{"detail": "<why>"}` |
| **500** | PDF backend unavailable (APIs 4 / 10 only) — fails that endpoint, not the app | generic |
...
```json
{
  "count": 14, "next": null, "previous": null,
  "results": {
    "message": "Orders fetched successfully",
    "counts": { "new": 3, "in_progress": 5, "express": 2, "delivered": 8,
                "failed": 1, "intent_received": 4, "all": 14 },
    "data": [
      { "id": "…", "order_number": "AM202608100001",
        "status": "At Berth", "status_code": "at_berth",
        "is_express": false, "is_fastest_delivery": true,
        "sailor_name": "Ravi Kumar", "imo_number": "9074729",
        "anchorage": "Outer Anchorage", "item_count": 5,
        "items_preview": "Mooring Rope, Deck Brush, …",
        "deliver_by": "July 21, 2026, 03:00 PM",
        "expected_departure": "2026-07-23T08:00:00Z",
        "created_at": "July 18, 2026, 09:12 AM",
...
**`counts` is computed over the partner's whole queue, not the current tab** — the header
badges must not change when a tab is opened. `intent_received` counts *pre-delivery* work
(Flow 06) and is deliberately outside `all`, which covers delivery-stage orders only.

`deliver_by` is **null for normal orders** — only SLA-bound ones (express / fastest) carry a
deadline; `expected_departure` is the guidance otherwise.

`has_surprise_gift` is **a flag and only ever a flag** (#28 v2.0). The system never knows what
the gift is — it is prepared off-system — so there is no identity to expose here or on the
detail screen. The partner only needs to know a wrapped, name-tagged parcel travels with the
order.

### API 2 — Success `200`

`PartnerOrderDetailSerializer` — the card fields above plus the delivery context (shipment
address, port/anchorage, ship agent, payment state, delta hold). Returned **bare**, not
wrapped in a `message`/`data` envelope.

### API 3 — Success `200`

```json
{
  "order_id": "…", "order_number": "AM202608100001", "item_count": 5,
  "items": [
    { "order_item_id": "…", "variant_id": "…",
      "product_name": "Mooring Rope", "sku": "ROPE-24MM",
      "quantity": 2, "unit_price": "120.00", "is_sourcable": true,
      "attributes": { "…": "…" }, "about_product": "…", "description": "…",
      "category": "Deck Stores", "catalog_type": "regular",
      "images": ["https://…"] }
  ]
}
```

Passing `?order_item_id=` returns a single line under `item` instead of the `items` array.

The **order's snapshot** is authoritative for `product_name` / `sku` / `unit_price` /
`quantity`; spec details (`attributes`, `about_product`, `images`) fall back to the live
variant, so a catalog edit after ordering never rewrites what the sailor was billed for.
...
**Success — 200** — `{ "message": "Assignment rejected.", "order_id": "…", "order_status": "partner_assigned" }`

`order_status` intentionally reports the **unchanged** value: rejection ends this partner's
involvement, not the order's progress.

**Errors** — 400 (blank `reason`; work already started —
`{"detail": "This assignment can no longer be rejected — work has already started."}`) ·
401 · 403 (not a partner) · 404 (not the caller's active assignment)
...
**Success — 200** — `{ "message": "Delivery marked as failed.", "order_status": "delivery_failed" }`

The assignment staying **active** is the deliberate part: closing it would leave the order with
no partner on the hook and nothing telling an admin to reassign (the same reasoning as the
re-verify loop in Flow 15).

**Errors** — 400 (blank `reason`) · **409** (`InvalidOrderTransition` — the order is not in a
state that can fail, e.g. already delivered) · 401 · 403 · 404
