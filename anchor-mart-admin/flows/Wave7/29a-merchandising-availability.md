...
| **Documentation Version** | 1.1 — 2026-08-07 (FE1, shipped 2026-08-06: `Product.is_internal` gives the hidden-quote meaning its own column — closes this flow's F-02). 1.0 — 2026-07-29 |
...
**There is now a discriminator** *(added 2026-08-06 — FE1, the fix for this flow's F-02)*.
Meaning (2) is carried by its own column, **`Product.is_internal`**, set by whatever creates the
private product — the special-request quote path and the partner-substitute holding pool. The
guards in `catalog.waitlist_service` read that column: **`public_products_q()`** (`Q(is_internal=False)`)
and **`public_variants_q()`** (`Q(product__is_internal=False)`), renamed from
`not_a_quote_product_q()` / `not_a_quote_variant_q()`.

Before this, the only marker was the presence of a `SuggestedProductByAdmin` row pointing at the
variant — which also matched an ordinary catalog SKU offered as a *substitute*, so offering SKU *X*
as a replacement on any order permanently removed *X*'s whole product from browse.

**Practical consequence for an admin UI:** `admin_sourceable=False` still carries meaning (1) only —
"cannot currently source". To decide whether a product is private, read **`is_internal`**. Any new
screen that lists products must exclude internal ones (`public_products_q()`), or it will leak one
sailor's bespoke quote into a general listing. **Nothing infers the flag** — a third creator of a
private product must set it itself.
...
1. The product must be **live and active**, and must **not** be an internal product — a bespoke
   quote or a partner-substitute holding product (`public_products_q()`) — else **404**.
