...
| Status | When | Body |
|---|---|---|
| **400** | An invalid filter — unknown `category` or `action`, malformed date, non-integer pagination. On **verify**: a missing `subject_type`/`subject_id` (`{"detail": "Both subject_type and subject_id are required."}`) or a `subject_type` outside `AuditLog.Subject` | DRF field map |
| **401** | Missing or expired token | `{"detail": "Authentication credentials were not provided."}` |
| **403** | A sub-admin requesting `category=operational`, **or any sub-admin calling verify** — `{"detail": "Chain verification is restricted to super admins."}` | `{"detail": "…"}` |
| **404** | `?page=` beyond the last page (list only) | `{"detail": "Invalid page."}` |

**403, not a filtered result, when a sub-admin asks for `operational`.** The tier boundary is
answered rather than silently narrowed: a sub-admin who requested operational entries and
received an empty page could reasonably conclude none exist, which is a different and false
statement. The list endpoint narrows only when no `category` is supplied.

Verification failure is **not** an error status, and neither is "no chain". Both return **200**:

```json
{ "subject_type": "order", "subject_id": "…",
  "verified": false, "error": "entry 4: prev_hash does not match entry 3",
  "entries": 12, "pruned_before": "2025-08-10T00:00:00Z" }
```

A tampered chain is a **finding to display**, not a transport failure — a 4xx/5xx would make it
indistinguishable from a bad request or an outage. A subject that has no chain at all returns
`verified: true` semantics with `entries: 0` rather than a 404; there is nothing to refuse.

**`pruned_before` is load-bearing.** Non-null means retention removed the earlier trail, so
verification starts from the first surviving entry — the endpoint says so rather than implying
it checked the full history.
