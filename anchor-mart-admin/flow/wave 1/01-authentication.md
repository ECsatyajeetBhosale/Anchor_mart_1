...
| **Services** | `generate_signin_otp`, `generate_whatsapp_otp`, `create_new_user`, `send_otp_email`, `record_auth_event`, `ExpiringTokenAuthentication` |
...
| `SigninOtp` | `user/models.py:233-250` · `SigninOtp` | Email OTP. **Keyed on the email string, not a user FK.** Fields: `email`, `otp`, `is_used`, `expires_at` |
| `WhatsappOtp` | `user/models.py:256-287` · `WhatsappOtp` | WhatsApp OTP. Keyed to a `user` FK. Has an `attempts` counter |
| `FcmToken` | `user/models.py:290-296` · `FcmToken` | Push token; `token` is globally unique |
...
| 400 | `{"message": "Otp is incorrect or already used"}` | No matching unused, active row |
| 400 | `{"message": "Otp is expired"}` | Matched but past `expires_at` |
...
| 400 | `{"message": "Otp is incorrect or already used"}` | No matching unused row |
