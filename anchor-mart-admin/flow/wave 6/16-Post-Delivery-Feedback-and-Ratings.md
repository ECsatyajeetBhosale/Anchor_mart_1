...
| **Endpoints** | **8** — 3 delivery-rating (`/api/orders/…`) · 2 app-rating (`/api/user/…`) · **3 admin** (`/api/superadmin/ratings/…`) |
...
## 4 · `POST /api/user/app-rating/submit/` — Submit or revise the app rating
...
## 5 · `GET /api/user/app-rating/` — The caller's current app rating (drives the prompt)
...
**Query params:** `days` (optional positive int → a rolling window of the last *N* days incl.
today; **400** if `< 1` or non-integer). Omitted → **all-time**.


**Response `200`:**
```json
{ "window": { "days": null, "start": "2020-01-01", "end": "2026-07-27" },
