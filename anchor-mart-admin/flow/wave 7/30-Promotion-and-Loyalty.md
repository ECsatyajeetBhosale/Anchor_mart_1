...
| **Documentation Status** | ✅ 15 routes fully specified. Routes taken from the running route table; **behaviour verified by EXECUTING every endpoint** against a real database (151 calls / observations, 2026-07-30) — not inferred from source. **Revised 2026-07-30** for the GB1 / GB3 / GB4 / GB5 / GB6 / GB7 / GB8 fixes, each locked by permanent tests in `admin_panel/tests/test_promotion_remediation.py` (48 tests; **36 verified red pre-fix**). |
...
`BonusPoints` row for the user regardless of type. The Loyalty Overview endpoint (§13),
however, reports on `loyalty` rows only.
...
Coupons are soft-deleted (`is_deleted=True`, `is_active=False`); every list and lookup filters
`is_deleted=False`, so a deleted coupon 404s on update and delete. **Coupon assignments are the
exception — `DELETE assignments/<id>/` is a hard delete**, the row is gone.
...
- `times_used` — the counter on the coupon row, incremented at redemption. (The report in §5
 derives its own count from the usage rows instead.)
...
| `min_purchase_amount` | decimal | ❌ | Default `0`. Minimum **items subtotal** before the coupon may be used. |
| `max_discount_amount` | decimal \| null | ❌ | Cap on the discount. **Only valid when `discount_type = percentage`** (400 otherwise) and **must be > 0**. `null` = no cap. Silently forced to `null` on save for non-percentage coupons. |
...
| `usage_limit` | int \| null | ❌ | Total redemptions across all users. `null` = unlimited. Must be ≥ 0. |
| `per_user_usage_limit` | int | ❌ | Default **1**. How many times one user may redeem it. Must be ≥ 0. |
...
Sets `is_deleted=True` and `is_active=False`. (`deleted_at` and `deleted_by` are **left null** —
the actor is recoverable from the audit entry instead; validation report **GB10**.)
...
| `times_used` | **`COUNT(CouponUsage)`** for this coupon — derived from the usage rows, *not* the `Coupon.times_used` counter that §1 returns. |
