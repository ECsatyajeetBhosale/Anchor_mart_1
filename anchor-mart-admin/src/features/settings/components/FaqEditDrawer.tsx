...
import { useGetFaqQuery, useGetFaqTypesQuery, useUpdateFaqMutation } from "../api/faqApi";
import { type FaqFormData, faqSchema } from "../schemas/settings.schema";
import type { Faq } from "../types/settings.types";

export interface FaqEditDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  faq: Faq;
}

export function FaqEditDrawer({ isOpen, onClose, faq }: FaqEditDrawerProps) {
  ...
  // Load the record fresh; fall back to the row while the request is in flight.
  const { data: detail } = useGetFaqQuery(faq.id, { skip: !isOpen });
  const source = detail ?? faq;
  ...
  const onSubmit = async (formData: FaqFormData) => {
    try {
      ...
      onClose();
      toast.success(getApiMessage(response) ?? MESSAGES.SETTINGS.FAQ.EDIT.TOAST_SUCCESS);
    } catch (error) {
      toast.error(getApiMessage(error) ?? MESSAGES.SETTINGS.FAQ.EDIT.TOAST_ERROR);
    }
  };
      ...
      >
        <SheetHeader className="p-6 pb-2 border-b border-[var(--border-md)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle className="text-xl">{MESSAGES.SETTINGS.FAQ.EDIT.TITLE}</SheetTitle>
              <SheetDescription>{MESSAGES.SETTINGS.FAQ.EDIT.SUBTITLE}</SheetDescription>
            </div>
          </div>
        </SheetHeader>
        ...
        <SheetFooter className="p-6 border-t border-[var(--border-md)] bg-[var(--surface)]">
          <div className="flex justify-end gap-3 w-full">
            ...
            >
              <IconCheck size={16} />
              {isLoading ? MESSAGES.SETTINGS.FAQ.EDIT.SAVING : MESSAGES.SETTINGS.FAQ.EDIT.SUBMIT}
            </button>
          </div>
        </SheetFooter>
      </SheetContent>
