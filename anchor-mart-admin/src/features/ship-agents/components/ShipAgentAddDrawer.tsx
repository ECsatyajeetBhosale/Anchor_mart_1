...
import { useCreateShipAgentMutation } from "../api/shipAgentApi";
import { type ShipAgentFormData, shipAgentSchema } from "../schemas/shipAgent.schema";
import type { ShipAgentPayload } from "../types/shipAgent.types";

const M = MESSAGES.SHIP_AGENTS;

export interface ShipAgentAddDrawerProps {
  isOpen: boolean;
  onClose: () => void;
}

const ADD_DEFAULTS: ShipAgentFormData = {
  name: "",
  company: "",
  country_code: "",
  mobile: "",
...
export function ShipAgentAddDrawer({ isOpen, onClose }: ShipAgentAddDrawerProps) {
  const [createShipAgent, { isLoading: isCreating }] = useCreateShipAgentMutation();
  ...
  const onSubmit = async (formData: ShipAgentFormData) => {
    ...
    try {
      const response = await createShipAgent(payload).unwrap();
      // Success: close the drawer first, then notify.
      onClose();
      toast.success(getApiMessage(response) ?? M.TOAST.ADD_SUCCESS);
    } catch (error) {
      ...
      >
        <SheetHeader className="p-6 pb-2 border-b border-[var(--border-md)]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <SheetTitle className="text-xl">{M.ADD.TITLE}</SheetTitle>
              <SheetDescription>{M.ADD.SUBTITLE}</SheetDescription>
            </div>
          </div>
        </SheetHeader>
        ...
        <SheetFooter className="p-6 border-t border-[var(--border-md)] bg-[var(--surface)]">
          <div className="flex justify-end gap-3 w-full">
            ...
            >
              <IconCheck size={16} />
              {isCreating ? M.ADD.SAVING : M.ADD.SUBMIT}
            </button>
          </div>
        </SheetFooter>
      </SheetContent>
