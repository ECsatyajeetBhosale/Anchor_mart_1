...
function Badge({
  ...
  const variantClasses = {
    ...
    blue: "bg-[var(--info-bg)]    border-[var(--info-border)]    text-[var(--info-text)]",
  };
  ...
  const dotClasses = {
    ...
    green: "bg-[var(--green-icon)]",
    blue: "bg-[var(--info-icon)]",
  };
  ...
  return (
    <div
      ...
      {...props}
    >
      {showDot && <span className={cn("w-1.5 h-1.5 rounded-full", dotClasses[variant])} />}
      {children}
    </div>
  );
}
